...
public final class NotificationManager {
    ...
    /**
     * Announces one AI pricing cycle's aggregated price-change summary
     * across every enabled channel (Revisi 20). Replaces the previous
     * per-item {@code announcePriceChange}, which broadcast one chat
     * message per changed item every cycle - for a catalog of any real
     * size that spammed chat non-stop. Now exactly one message goes
     * out per cycle, showing the overall average percent change.
     *
     * @param summary the aggregated cycle summary to announce
     */
    public void announcePriceCycleSummary(io.azthera.ecocore.ai.AiEconomyEngine.PriceCycleSummary summary) {
        if (!configManager.isBroadcastPriceChanges() || summary.itemsChanged() == 0) {
            return;
        }

        boolean up = summary.averagePercentChange() >= 0;
        String messageKey = up ? "market.cycle-summary-up" : "market.cycle-summary-down";
        String topItemId = summary.topMover() != null ? summary.topMover().getId() : "-";
        String message = messagesConfig.getWithPrefix(messageKey,
                "count", String.valueOf(summary.itemsChanged()),
                "percent", String.format("%.1f", Math.abs(summary.averagePercentChange())),
                "topItem", topItemId,
                "topPercent", String.format("%.1f", Math.abs(summary.topMoverPercent())));
        ...
        if (configManager.isUseDiscordNotifications()) {
            discordBotManager.sendEmbed(DiscordChannelRouter.CHANNEL_MARKET, "PRICE_CHANGE",
                    embedBuilder.priceCycleSummaryEmbed(summary));
