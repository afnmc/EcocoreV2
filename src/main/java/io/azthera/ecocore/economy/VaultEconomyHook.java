...
public final class VaultEconomyHook implements Economy {
    ...
    public EconomyResponse depositPlayer(OfflinePlayer player, double amount) {
        UUID uuid = player.getUniqueId();
        boolean success = economyEngine.deposit(uuid, amount, TransactionLogger.REASON_ADMIN_ADJUST);
        double balance = economyEngine.getBalance(uuid);
        return success
                ? new EconomyResponse(amount, balance, EconomyResponse.ResponseType.SUCCESS, "")
                : new EconomyResponse(0, balance, EconomyResponse.ResponseType.FAILURE, "Failed to deposit into database");
    }
