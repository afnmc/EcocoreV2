...
public final class ShopItemPreviewGui extends AbstractGui {
    ...
    @Override
    public void build() {
        inventory = Bukkit.createInventory(this, 36, messagesConfig.get("ui.shop.preview-title"));
        render();
    }
    ...
    private ItemStack buildPreviewIcon(ShopItemRecord item) {
        ...
        if (meta != null) {
            ...
            String priceChangeLine = io.azthera.ecocore.utils.ItemUtils.priceChangeLoreLine(item);
            if (priceChangeLine != null) {
                lore.add(priceChangeLine);
            }
    ...
    private ItemStack buildBuyButton(ShopItemRecord item, int amount) {
        ...
        ItemMeta meta = icon.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(messagesConfig.get("ui.shop.buy", "amount", String.valueOf(amount)));
            meta.setLore(List.of(messagesConfig.get("ui.common.total", "total", String.format("%.2f", total))));
            icon.setItemMeta(meta);
        }
        return icon;
    }
    ...
    private ItemStack buildBuyMaxButton(ShopItemRecord item) {
        ...
        ItemMeta meta = icon.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(messagesConfig.get("ui.shop.buy-max", "stock", String.valueOf(item.getStock())));
