...
public final class ShopBuyConfirmGui extends AbstractGui {
    ...
    @Override
    public void build() {
        inventory = Bukkit.createInventory(this, 27, messagesConfig.get("ui.shop.confirm-title"));

        ItemStack info = new ItemStack(Material.PAPER);
        ItemMeta infoMeta = info.getItemMeta();
        if (infoMeta != null) {
            infoMeta.setDisplayName(messagesConfig.get("ui.shop.confirm-question", "amount", String.valueOf(amount), "item", itemId));
            infoMeta.setLore(List.of(messagesConfig.get("ui.common.total", "total", String.format("%.2f", totalPrice))));
        ...
        if (confirmMeta != null) {
            confirmMeta.setDisplayName(messagesConfig.get("ui.common.confirm"));
        ...
        if (cancelMeta != null) {
            cancelMeta.setDisplayName(messagesConfig.get("ui.common.cancel"));
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (slot == CONFIRM_SLOT) {
            ...
            if (result.success()) {
                ...
                // Map each specific failure reason to the correct user-facing message
                String messageKey = switch (result.message()) {
                    case "sold-out" -> "shop.sold-out";
                    case "insufficient-funds" -> "economy.not-enough-money";
                    case "stock-update-failed" -> "shop.stock-error";
                    case "database-error" -> "shop.database-error";
                    case "not-tradeable" -> "shop.not-tradeable";
                    case "player-offline" -> "shop.player-offline";
                    case "invalid-amount" -> "shop.invalid-amount";
                    default -> "shop.purchase-failed";
                };
                viewer.sendMessage(messagesConfig.getWithPrefix(messageKey));
