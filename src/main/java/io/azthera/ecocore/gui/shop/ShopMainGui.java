...
public final class ShopMainGui extends AbstractGui {
    ...
    private ItemStack buildCategoryIcon(ShopCategory category) {
        ItemStack icon = null;
        if (category.getIconData() != null && !category.getIconData().isBlank()) {
            ItemStack[] decoded = io.azthera.ecocore.utils.ItemUtils.deserialize(
                    java.util.logging.Logger.getLogger("EcoCore"), category.getIconData(), 1);
            if (decoded.length > 0 && decoded[0] != null) {
                icon = decoded[0].clone();
                icon.setAmount(1);
            }
        }
        if (icon == null) {
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (slot == SEARCH_SLOT && shopConfig.isSearchEnabled()) {
            ...
            PrivateChatInputManager input = EcoCorePlugin.getInstance().getPrivateChatInputManager();
            input.request(viewer, query -> {
                ShopSearchGui resultGui = new ShopSearchGui(viewer, shopManager, shopConfig, guiManager, guiConfig, messagesConfig);
                guiManager.register(viewer, resultGui);
                resultGui.showResults(shopManager.search(query), query);
            }, () -> {
                ShopMainGui main = new ShopMainGui(viewer, shopManager, shopConfig, guiManager, guiConfig, messagesConfig);
                guiManager.register(viewer, main);
                main.open();
            });
            return;
        }
