...
public final class AdminMainGui extends AbstractGui {
    ...
    public void build() {
        ...
        set(16, Material.WRITABLE_BOOK, "&fPlayer Data", "&7Cari saldo dan data player");
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        switch (event.getRawSlot()) {
            case 10 -> viewer.sendMessage(color("&6[EcoCore] &fEconomy aktif. Gunakan Player Data untuk saldo player dan Configuration untuk reload."));
            case 11 -> openShopEditor();
            case 12 -> openNightMarketEditor();
            ...
            case 16 -> {
                ...
                viewer.sendMessage(color("&b[EcoCore] &fKetik nama player di chat untuk melihat saldo, atau &ccancel&f."));
                inputManager.request(viewer, name -> {
                    Player target = Bukkit.getPlayerExact(name.trim());
                    if (target == null) viewer.sendMessage(color("&c[EcoCore] Player harus online."));
                    else viewer.sendMessage(color("&6[EcoCore] &f" + target.getName() + " &7saldo: &a$" + String.format("%.2f", EcoCorePlugin.getInstance().getEconomyEngine().getBalance(target.getUniqueId()))));
                    open();
                }, this::open);
            }
            case 15 -> {
    ...
    private void openNightMarketEditor() {
        AdminNightMarketGui gui = new AdminNightMarketGui(viewer, guiManager, inputManager);
        guiManager.register(viewer, gui);
        gui.open();
    }
