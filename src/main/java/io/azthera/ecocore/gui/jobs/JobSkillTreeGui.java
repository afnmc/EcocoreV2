...
public final class JobSkillTreeGui extends AbstractGui {
    ...
    @Override
    public void build() {
        inventory = Bukkit.createInventory(this, 54, messagesConfig.get("ui.jobs.skill-title", "job", jobType.configKey()));
        ...
        try {
            ...
            viewer.sendMessage(messagesConfig.getWithPrefix("ui.jobs.skill-failed"));
        }
    ...
    private ItemStack buildNodeIcon(SkillTreeNode node, boolean unlocked) {
        ...
        if (meta != null) {
            meta.setDisplayName((unlocked ? "§a" : "§7") + messagesConfig.get("ui.jobs.skill-bonus", "bonus", node.bonusType()));
