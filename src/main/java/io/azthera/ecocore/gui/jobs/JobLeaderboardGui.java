...
public final class JobLeaderboardGui extends AbstractGui {
    ...
    @Override
    public void build() {
        inventory = Bukkit.createInventory(this, 54, io.azthera.ecocore.EcoCorePlugin.getInstance().getMessagesConfig().get("ui.jobs.leaderboard-title", "job", jobType.configKey()));
        ...
        try {
            ...
            viewer.sendMessage(io.azthera.ecocore.EcoCorePlugin.getInstance().getMessagesConfig().getWithPrefix("ui.jobs.leaderboard-failed"));
        }

        inventory.setItem(BACK_SLOT, guiManager.buildButtonIcon("back", io.azthera.ecocore.EcoCorePlugin.getInstance().getMessagesConfig().get("ui.common.back")));
    }
