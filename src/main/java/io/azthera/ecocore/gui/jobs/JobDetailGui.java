...
public final class JobDetailGui extends AbstractGui {
    ...
    /** Set after the first click on {@link #LEAVE_SLOT}; a second click confirms leaving. */
    private boolean pendingLeaveConfirmation;
    ...
    @Override
    public void build() {
        inventory = Bukkit.createInventory(this, 54, messagesConfig.get("ui.jobs.detail-title", "job", jobType.configKey()));
        render();
    }
    ...
    private void render() {
        ...
        try {
            ...
            inventory.setItem(LEAVE_SLOT, buildLeaveIcon());

            ItemStack missionHeader = new ItemStack(Material.WRITTEN_BOOK);
            ItemMeta headerMeta = missionHeader.getItemMeta();
            if (headerMeta != null) {
                headerMeta.setDisplayName(messagesConfig.get("ui.jobs.missions-title"));
                headerMeta.setLore(List.of(messagesConfig.get("ui.jobs.missions-lore")));
            ...
            if (missions.isEmpty()) {
                ...
                if (noneMeta != null) {
                    noneMeta.setDisplayName(messagesConfig.get("ui.jobs.no-missions"));
                    noneMeta.setLore(List.of(messagesConfig.get("ui.jobs.no-missions-lore")));
                    noneIcon.setItemMeta(noneMeta);
                }
                inventory.setItem(MISSIONS_START_SLOT, noneIcon);
            }
        } catch (SQLException exception) {
            viewer.sendMessage(messagesConfig.getWithPrefix("ui.jobs.load-failed"));
        }
    ...
    private ItemStack buildPrestigeIcon(JobData data, boolean canPrestige) {
        ...
        ItemMeta meta = icon.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(canPrestige ? messagesConfig.get("ui.jobs.prestige-available") : messagesConfig.get("ui.jobs.prestige"));
    ...
    private ItemStack buildLeaveIcon() {
        ItemStack icon = new ItemStack(pendingLeaveConfirmation ? Material.TNT : Material.BARRIER);
        ItemMeta meta = icon.getItemMeta();
        if (meta != null) {
            if (pendingLeaveConfirmation) {
                meta.setDisplayName(messagesConfig.get("ui.jobs.leave-question"));
                meta.setLore(List.of(
                        "§7Klik lagi buat konfirmasi keluar dari job ini.",
                        "§7Level, XP, dan prestige job ini akan hilang.",
                        "§8Klik tombol lain buat batal."));
            } else {
                meta.setDisplayName(messagesConfig.get("ui.jobs.leave-title"));
                meta.setLore(List.of(
                        "§7Klik untuk keluar dari job " + jobType.configKey() + ".",
                        "§7Lu perlu klik &fdua kali &7buat konfirmasi.".replace("&f", "§f")));
            }
            icon.setItemMeta(meta);
        }
        return icon;
    ...
    private ItemStack buildMissionIcon(JobMissionRecord mission) {
        ...
        if (meta != null) {
            ...
            io.azthera.ecocore.jobs.MissionTemplate.DecodedKey decoded =
                    io.azthera.ecocore.jobs.MissionTemplate.decode(mission.missionKey());
            String instruction = io.azthera.ecocore.jobs.MissionDescriber.describe(
                    decoded != null ? decoded.type() : null,
                    decoded != null ? decoded.target() : null,
                    mission.target());

            List<String> lore = new ArrayList<>();
            lore.add("§7Tugas: §f" + instruction);
            lore.add("");
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (slot != LEAVE_SLOT && pendingLeaveConfirmation) {
        ...
        if (slot == PRESTIGE_SLOT) {
            try {
                ...
                viewer.sendMessage(messagesConfig.getWithPrefix("ui.jobs.prestige-failed"));
            }
            return;
        }
        ...
        if (slot == LEAVE_SLOT) {
            ...
            try {
                jobsManager.leave(viewer.getUniqueId(), jobType);
                viewer.sendMessage(messagesConfig.getWithPrefix("jobs.left", "job", jobType.configKey()));
                guiManager.playSound(viewer, "click");
            } catch (SQLException exception) {
                viewer.sendMessage(messagesConfig.getWithPrefix("ui.jobs.leave-failed"));
                return;
            }
