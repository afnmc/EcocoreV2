...
public final class DiscordEmbedBuilder {
    ...
    /**
     * Builds an embed announcing an aggregated price-change summary
     * for a whole AI cycle (Revisi 20) - replaces sending one embed
     * per item, which spammed the Discord channel on every cycle for
     * servers with a large catalog.
     *
     * @param summary the cycle summary to announce
     * @return the built embed
     */
    public EmbedBuilder priceCycleSummaryEmbed(io.azthera.ecocore.ai.AiEconomyEngine.PriceCycleSummary summary) {
        boolean up = summary.averagePercentChange() >= 0;
        EmbedBuilder embed = base()
                .setTitle((up ? "\uD83D\uDCC8 " : "\uD83D\uDCC9 ") + "Update Harga Pasar")
                .setColor(parseColor(up ? discordConfig.getColorPriceUp() : discordConfig.getColorPriceDown()))
                .addField("Item Berubah", String.valueOf(summary.itemsChanged()), true)
                .addField("Rata-rata Perubahan", String.format("%+.2f%%", summary.averagePercentChange()), true);
        if (summary.topMover() != null) {
            embed.addField("Paling Bergerak", summary.topMover().getId()
                    + String.format(" (%+.2f%%)", summary.topMoverPercent()), true);
        }
        return embed;
    }
