...
public final class JobProgressTracker {
    ...
    public ActionResult applyAction(UUID playerUuid, JobHandler handler, String actionKey,
        ...
        return applyAction(playerUuid, handler, actionKey, jobBonusMultiplier, 1);
    }

    /**
     * Applies an action with a concrete quantity. A quantity of 4 crafted
     * items, 32 sold items, etc. awards four/32 normal action units.
     */
    public ActionResult applyAction(UUID playerUuid, JobHandler handler, String actionKey,
                                     double jobBonusMultiplier, int quantity) throws SQLException {
        ...
        double money = definition.baseMoneyPerAction() * multiplier * jobBonusMultiplier * prestigeBonus * weight;
        long xpGain = Math.round(definition.baseXpPerAction() * multiplier * jobBonusMultiplier * prestigeBonus * weight);
