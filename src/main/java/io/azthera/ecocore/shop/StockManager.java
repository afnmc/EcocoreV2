...
public final class StockManager {
    ...
    private boolean applyChange(String itemId, int delta, String eventType) {
        ...
        synchronized (item) {
            ...
            long beforeUpdatedAt = item.getUpdatedAt();
            long beforeLastRestockAt = item.getLastRestockAt();
            int beforeRestocksToday = item.getRestocksToday();
            long beforeRestockDayEpoch = item.getRestockDayEpoch();
            ...
            try {
                shopItemDao.updateStockAndEventAtomic(item, after - before, eventType, isThrottledRestockEvent);
                return true;
            } catch (SQLException exception) {
                logger.severe("[EcoCore] Failed to persist stock change for " + itemId + ": " + exception.getMessage());
                item.setStock(before);
                item.setUpdatedAt(beforeUpdatedAt);
                item.setLastRestockAt(beforeLastRestockAt);
                item.setRestocksToday(beforeRestocksToday);
                item.setRestockDayEpoch(beforeRestockDayEpoch);
                return false;
            }
        }
    }
