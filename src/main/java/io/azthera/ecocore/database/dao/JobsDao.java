...
public final class JobsDao {
    ...
    /**
     * Deletes a player's progress in a single job (Revisi 20, used by
     * {@code /jobs} "Leave Job" button) - level, xp, and prestige for
     * that job are permanently lost; active missions for that job
     * should be cleared separately via {@code JobMissionDao}.
     *
     * @param playerUuid the player's uuid
     * @param jobType    the job type to remove
     * @throws SQLException if the delete fails
     */
    public void delete(UUID playerUuid, JobType jobType) throws SQLException {
        String sql = "DELETE FROM jobs_data WHERE player_uuid = ? AND job_type = ?";
        try (Connection connection = databaseManager.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setString(1, playerUuid.toString());
            statement.setString(2, jobType.name());
